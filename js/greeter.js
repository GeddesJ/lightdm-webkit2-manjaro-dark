...
class AntergosThemeUtils {
	...
	init_config_values() {
		...
		if ( 'undefined' !== typeof( config ) ) {
			...
			logo = config.get_str( 'branding', 'logo' ) || 'img/manjaro.png';
			user_image = config.get_str( 'branding', 'user_image' ) || 'img/manjaro-logo-user.png';
...
class AntergosTheme {
	...
	constructor() {
		...
		this.tux = 'img/manjaro-logo-user.png';
	...
	prepare_user_list() {
		...
		for ( var user of lightdm.users ) {
			...
			$( template ).appendTo( this.$user_list ).click( this.start_authentication ).on( 'error.manjaro', this.user_image_error_handler );
	...
	prepare_login_panel_header() {
		var greeting = (_util.translations.greeting) ? _util.translations.greeting : 'Welcome!',
			logo = ( '' !== _util.logo ) ? _util.logo : 'img/manjaro.png';
